...
export default function SolarCrossSell({
  ...
  return (
    <section
      ...
      className="mb-10 flex flex-col items-start gap-3 rounded-xl border border-spark-teal/25 bg-spark-teal/5 p-6 sm:flex-row sm:items-center sm:justify-between dark:border-spark-teal/20 dark:bg-spark-teal/10"
    >
      <div>
        <h2
          id="solar-crosssell"
          className="font-display text-lg font-semibold text-ink-navy dark:text-white"
        >
          ☀️ See how much solar could save you
          {state ? ` in ${state}` : ''}
        </h2>
        <p className="mt-1 text-sm text-ash/70 dark:text-gazette-cream/70">
